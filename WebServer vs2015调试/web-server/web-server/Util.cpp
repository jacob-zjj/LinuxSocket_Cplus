...
//设置读函数的几种类型 主要是针对buff的形式而言 有的时候
...
void setSocketNodelay(int fd)
{
	...
	这样可以降低包的传输个数。同时协议也要求提供一个方法给上层来禁止掉Nagle算法

	当你的应用不是连续请求+应答的模型的时候，而是需要实时的单项的发送数据并及时获取响应，这种case就明显不太适合Nagle算法，
	明显有delay的。
...
void setSocketNoLinger(int fd)
{
	...
	3. l_onoff != 0; l_linger > 0;
		...
		否则，close()会直接返回错误值，未发送数据丢失，socket描述符被强制性退出。需要注意的是，
...
//--以上服务于对地址地址进行解码和编码，由于输入的地址是中文时，前台传过来的地址数据需要进行解码才能识别------------------------------------------------------------------------------